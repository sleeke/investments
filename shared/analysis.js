const { daily } = require('../bargainHunt/network/iexService');
...
module.exports.calculatePremarketCorrelation = function(dailyData, symbolAnalysisOutput) {
  return new Promise(function(resolve, reject) {
    ...
    var lastClose = -1
    var numCorrelated = 0
    var numNotCorrelated = 0
    var accumulatedCorrelationPercent = 0

    var specificPremarketRangePercent = {
      min: 1, 
      max: 1.5
    }
    var numCorrelatedWithinRange = 0
    var numNotCorrelatedWithinRange = 0
    var accumulatedCorrelationWithinRangePercent = 0

    for (index = dailyData.length - 1; index >= 0 ; index--) {
      if (lastClose > 0) {
        var premarketGainPercent = (dailyData[index].open / lastClose - 1) * 100;
        var gainFromOpenPercent = (dailyData[index].high / dailyData[index].open - 1) * 100
  
        if (premarketGainPercent > 0 && gainFromOpenPercent > 0
          || premarketGainPercent < 0 && gainFromOpenPercent < 0) {
          numCorrelated++
          accumulatedCorrelationPercent += Math.abs(gainFromOpenPercent)
        }
        else {
          numNotCorrelated++
          accumulatedCorrelationPercent -= Math.abs(gainFromOpenPercent)
        ...
        // specific gain
        if ((premarketGainPercent > specificPremarketRangePercent.min && premarketGainPercent < specificPremarketRangePercent.max && gainFromOpenPercent > 0)
          || (premarketGainPercent > -specificPremarketRangePercent.max && premarketGainPercent < -specificPremarketRangePercent.min && gainFromOpenPercent < 0)) {
          numCorrelatedWithinRange++
          accumulatedCorrelationWithinRangePercent += Math.abs(gainFromOpenPercent)
        }
        else {
          numNotCorrelatedWithinRange++
          accumulatedCorrelationWithinRangePercent -= Math.abs(gainFromOpenPercent)
        }
      }

      lastClose = dailyData[index].close
    }
    
    symbolAnalysisOutput.basic = {}
    symbolAnalysisOutput.basic.correlationRatio = numCorrelated / numNotCorrelated
    symbolAnalysisOutput.basic.accumulatedCorrelationPercent = accumulatedCorrelationPercent
    symbolAnalysisOutput.basic.averageCorrelationPercent = accumulatedCorrelationPercent / dailyData.length

    symbolAnalysisOutput.withinRange = {}
    symbolAnalysisOutput.withinRange.range = { min: specificPremarketRangePercent.min, max: specificPremarketRangePercent.max }
    symbolAnalysisOutput.withinRange.correlationRatio = numCorrelatedWithinRange / numNotCorrelatedWithinRange
    symbolAnalysisOutput.withinRange.accumulatedCorrelationPercent = accumulatedCorrelationWithinRangePercent
    symbolAnalysisOutput.withinRange.averageCorrelationPercent = accumulatedCorrelationWithinRangePercent / dailyData.length
